...
let state = {month:'', days:[], holidays:new Set(), dutyDates:new Set(), olds:[...DEFAULT_OLDS], news:[...DEFAULT_NEWS], firstDuty:'', firstDay:{}, result:null, modified:false};
...
function renderPeople(){
  ...
  // The user always chooses the person who will take the month's first ACTUAL duty date,
  // even when day 1 itself is not a duty day.
  $('firstDutyField').classList.remove('hidden');
  $('firstDuty').innerHTML=`<option value="">请选择</option>` + names.map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join('');
  $('firstDuty').value=names.includes(state.firstDuty) ? state.firstDuty : '';
...
function renderFirstDay(){
  const body=$('firstDayBody'); body.innerHTML='';
  const names=allPeople();
  const firstIsDuty=state.days.length>0 && state.dutyDates.has(state.days[0]);
  names.forEach(n=>{
    // If day 1 is a duty day, the selected first-duty person is already fixed as D
    // and therefore does not appear in the manual day-1 list.
    if(firstIsDuty && n===state.firstDuty){
      state.firstDay[n]='D';
      return;
    }
    // Manual day-1 choices never include duty.
    if(state.firstDay[n]==='D') delete state.firstDay[n];
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${esc(n)}</td><td><select data-name="${esc(n)}"><option value="">请选择</option><option value="W">白班</option><option value="N">下夜</option><option value="R">休息</option></select></td>`;
    const sel=tr.querySelector('select');
    sel.value=(state.firstDay[n] && state.firstDay[n]!=='D') ? state.firstDay[n] : '';
    sel.onchange=()=>{state.firstDay[n]=sel.value; updatePrecheck()};
    body.appendChild(tr);
  });
}
function syncFirstDay(){renderFirstDay();const s=$('firstDuty');if(s) s.value=state.firstDuty;}
function updatePrecheck(){
  const names=allPeople(); const msgs=[];
  const first=state.days[0]; const firstIsDuty=!!first && state.dutyDates.has(first);
  if(names.length===0)msgs.push(['error','至少需要1名人员。']);
  if(state.dutyDates.size===0 && state.days.length>0)msgs.push(['error','当月至少需要1个值班日。']);

  // Always require the first actual duty day's duty person.
  if(!state.firstDuty || !names.includes(state.firstDuty)) msgs.push(['error','请先选择当月第一个实际值班日的值班人员。']);

  // Day 1 never allows manual duty. If day 1 itself is a duty day, the selected first-duty person is fixed as D and omitted.
  const manualNames=firstIsDuty ? names.filter(n=>n!==state.firstDuty) : names;
  manualNames.forEach(n=>{if(!state.firstDay[n])msgs.push(['warn',`请填写${n}的1号班次。`]);});
  if(firstIsDuty && state.firstDuty) state.firstDay[state.firstDuty]='D';

  // Sanitize stale D selections on manually editable people.
  manualNames.forEach(n=>{if(state.firstDay[n]==='D') delete state.firstDay[n];});
...
function buildDutySequence(){
  ...
  let start=rounds.indexOf(state.firstDuty); if(start<0) start=0;
...
function solve(){
  ...
  // 所有固定休息（1号人工休息 + 值班后的自动休息）都直接计入最终结果。
  const fixedRest = Array(P).fill(0);
  state.days.forEach(ds=>{
    names.forEach((p,i)=>{
      if(fixed[ds] && fixed[ds][p] === 'R') fixedRest[i]++;
    });
  });

  // 自动部分只需要补足到H；固定休息已经达到/超过H时，不再要求额外休息，也不报错。
  const restTarget = fixedRest.map(r=>Math.max(0,H-r));
  const rad=restTarget.map(x=>x+1);
  const mult=Array(P).fill(1);
  for(let i=1;i<P;i++)mult[i]=mult[i-1]*rad[i-1];
  const totalStates=mult[P-1]*rad[P-1];
  if(totalStates>3000000)throw new Error('人员数量过多或法定休息日过多，当前浏览器不建议计算。');
  ...
  for(let di=0;di<state.days.length;di++){
    const ds=state.days[di], wd=weekday(ds), holiday=state.holidays.has(ds);
    const dutyName=Object.entries(fixed[ds]||{}).find(([,v])=>v==='D')?.[0];
    // 1号完全由人工填写/首日值班选择决定，不参与任何白班最低人数或节假日白班校验。
    const minWhite = di===0 ? 0 : (holiday ? ((dutyName && isOld(dutyName)) ? 0 : 1) : ([1,2,3,5].includes(wd)?2:(wd===4?1:0)));
    const opts=[]; function rec(i,adds,white,oldWhite){
      if(i===P){
        if(white<minWhite) return;
        if(di!==0 && holiday && dutyName && !isOld(dutyName) && oldWhite<1)return;
        opts.push({adds:adds.slice(),white});return;
      }
      const fx=fixedType(ds,names[i]); if(fx){adds[i]=0; rec(i+1,adds,white+(fx==='W'?1:0),oldWhite+(fx==='W'&&isOld(names[i])?1:0));return}
    ...
    for(const [code,entry] of cur){
      for(const o of opts){
        let nc=code,ok=true;
        for(let i=0;i<P;i++){
          const c=Math.floor(nc/mult[i])%rad[i]; const n=c+o.adds[i];
          if(n>restTarget[i]){ok=false;break}
  ...
  let code=0;
  for(let i=0;i<P;i++) code += restTarget[i] * mult[i];
  const end=cur.get(code);
  if(!end)throw new Error('在保留人工填写班次的情况下，找不到完整的自动排班结果。请检查值班日期或白班最低人数设置。');
...
function shiftCellClass(x){return x==='D'?'cell-duty':x==='N'?'cell-night':x==='R'?'cell-rest':'cell-white'}
function updateSummary(){
  const names=allPeople(); const res=state.result; const sm=$('summary');
  sm.innerHTML='';
  names.forEach(p=>{
    let r=0,d=0,n=0,w=0;
    state.days.forEach(ds=>{
    ...
    const e=document.createElement('div');
    e.className='stat';
    e.innerHTML=`<b>${esc(p)}</b><span>值班 ${d} · 白班 ${w} · 下夜 ${n} · 休息 ${r}</span>`;
    sm.appendChild(e);
  });
}
function renderResult(){
  const names=allPeople(); const res=state.result; $('resultHint').textContent=`${state.month} · ${state.days.length}天 · 法定休息日${state.holidays.size}天 · 值班日${state.dutyDates.size}`;
  $('resultAlert').innerHTML='<div class="alert ok">排班生成成功。点击任意班次单元格即可人工调整；调整后的结果会直接用于下载Excel和CSV。</div>';
  updateSummary();
  const t=$('resultTable');
  ...
  names.forEach(p=>{
    ...
    state.days.forEach(ds=>{
      const x=res[ds][p];
      h+=`<td class="${shiftCellClass(x)} editable-cell" data-date="${ds}" data-person="${encodeURIComponent(p)}" title="点击修改">${SHIFT[x]}</td>`;
    });
    h+='</tr>';
  });
  h+='</tbody>';
  t.innerHTML=h;
  t.querySelectorAll('.editable-cell').forEach(cell=>cell.addEventListener('click',()=>openShiftEditor(cell.dataset.date,decodeURIComponent(cell.dataset.person))));
  $('modifiedBadge').classList.toggle('hidden',!state.modified);
}
function openShiftEditor(ds,person){
  const overlay=$('editOverlay'); const x=state.result[ds][person];
  $('editTitle').textContent=`${fmtDate(ds)} · ${person}`;
  $('editSubtitle').textContent=`当前：${SHIFT[x]} · 选择新的班次`;
  overlay.dataset.date=ds; overlay.dataset.person=encodeURIComponent(person); overlay.classList.remove('hidden');
}
function closeShiftEditor(){$('editOverlay').classList.add('hidden');}
function applyShift(shift){
  const overlay=$('editOverlay'); const ds=overlay.dataset.date, person=decodeURIComponent(overlay.dataset.person||''); if(!ds||!person)return;
  state.result[ds][person]=shift;
  state.modified=true;
  closeShiftEditor();
  renderResult();
  // Recalculate personnel shift counts immediately from the edited result.
  updateSummary();
  $('resultAlert').innerHTML='<div class="alert warn">排班结果已人工调整。人员班次统计已实时更新；当前下载的Excel/CSV将使用调整后的结果。</div>';
}
...
$('startBtn').onclick=()=>{const v=$('monthInput').value;if(!v){alert('请选择月份');return}state.month=v;buildDays();initHolidays();initDutyDates();state.firstDay={};state.firstDuty='';state.result=null;renderHolidayCalendar();showStep(2)};
...
// 使用事件委托，解决index.html中弹框DOM位于main.js之后导致按钮未绑定的问题。
document.addEventListener('click', e => {
  const option = e.target.closest('.edit-option');
  if(option){
    applyShift(option.dataset.shift);
    return;
  }
...
$('restartBtn').onclick=$('againBtn').onclick=()=>{state={month:'',days:[],holidays:new Set(),dutyDates:new Set(),olds:[...DEFAULT_OLDS],news:[...DEFAULT_NEWS],firstDuty:'',firstDay:{},result:null,modified:false};initMonth();showStep(1)};
initMonth();
